...
1. Confirm the user understands the tradeoff:

   > Solo mode disables worktree isolation. Code changes will go directly
   > to the main tree — no review checkpoint between subagent output and
   > your main branch. Recommended for solo projects only.
   >
   > Commit and push regularly so GitHub is your rollback safety net.
   >
   > Continue? [y/N]

2. On confirmation, write `solo\n` to `.github/.agent0-mode`. Create the file (and parent dir if needed) if it doesn't exist.

3. Note: this file is NOT in the framework manifest — it's per-project state. If the user doesn't want it committed, suggest they add it to `.gitignore`.
