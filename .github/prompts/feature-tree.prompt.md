...
- **Flair** — the bullet list under `design.md` `## Flair`, if that section exists (it's created by the Scaffold overlay's editor; absent in most specs). These are the changeable choices — the deliberate opposite of invariants. Empty/absent → empty array.
...
- `nodes`: one entry per feature directory (Phase 2/2a), PLUS one per stub node from `_nodes.yml`. For feature directories, map fields straight across: `id`, `kind`, `room` (null if ungrouped), `title`, `summary`, `detail`, `status`, `surfaces`, `invariants`, `flair` (from `design.md` `## Flair`, else `[]`), `dependsOn` (from `depends_on`), `todo`, and `artifacts`. For **stub nodes** (from `_nodes.yml`): emit `id`, `kind`, `room`, `title`, `summary`, `dependsOn` (from `depends_on`), `status` (default `built`), `verification: null`, empty `todo`/`invariants`/`flair`/`surfaces`, and `artifacts: { files: {} }`. **A stub whose id matches a real spec directory is dropped** — the spec wins (don't emit both).
