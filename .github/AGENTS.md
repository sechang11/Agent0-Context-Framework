...
- **Link MD files in chat output.** Whenever an Agent0-generated `.md` file is mentioned in chat (orchestrator summaries, dispatch suggestions, next-step hints), format the path as a **Markdown link** — `[.github/specs/auth-flow/verification.md](.github/specs/auth-flow/verification.md)` — so the user can click to preview without copying the path. Applies to: spec files (`requirements.md`, `design.md`, `tasks.md`, `verification.md`, `ui.md`), bug reports under `.github/bugs/`, `PROGRESS_REPORT.md`, `NEXT_STEPS.md`, theme files under `.github/themes/`, `responsive-strategy.md`, framework docs (`docs/*.md`, `CHANGELOG.md`), and any user-facing MD artifact Agent0 generates. Bare paths inside fenced code blocks don't render as links — break them out into a regular-Markdown "**Files:**" or "**Written to:**" list after the code-block summary. The user shouldn't have to copy paths manually.
