...
- **Knowledge artifacts always land in the main tree, never a worktree.** Files generated by Agent0 commands fall into two categories — *knowledge artifacts* (project-level documents and metadata) and *code changes* (implementation work). Code changes belong in the worktree where they can be reviewed before merging. Knowledge artifacts belong in the main tree so every conversation — present and future, in any worktree — can see them immediately. See the "Knowledge artifacts and worktrees" section below for the list and the implementation rule.
