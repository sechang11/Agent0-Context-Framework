...
- **This page** tab: node card (status, room, kind, verification X/Y, todo counts, depends-on chips) → action row (Cover / Verify / Spec — each queues a request) → **Invariants** section → **Flair** section, each collapsible, each editable (one item per line) with Save.
...
| `POST /__scaffold/save` | route | Body `{ node, section: "invariants"\|"flair", items: string[] }`. If the node has a spec dir (`.github/specs/{node}/`): surgically rewrite the owned section (bullet list, one `- item` per line) per safety property 4, append a `{ action: "spec-edited", node, section }` entry to the queue so Claude reviews the change next session, respond `{ ok, mode: "saved" }`. If it's a stub (no spec dir): append a `{ action: "note", node, note: <items joined> }` request instead and respond `{ ok, mode: "queued" }`. |
