...
| `/standup` | Instant terminal digest computed from `FEATURE_TREE.json` — In progress, Ready to pick up, Needs attention, Scaffold requests, Keystones, Dependency cycles, Coverage. The in-chat twin of the canvas **Project pulse**; run it at the start of a session. Optional `{room}` scope and `--full`. Read-only — for AI-judged priorities use `/nextsteps` |
| `/install-scaffold` | Erect the **Scaffold** — in-app, dev-only spec devtools: a pill → drawer on every page showing that page's node, its **Invariants** and **Flair** in an IDE-colored ELI5 editor, an all-specs browser, and a request queue Claude reads at the next `/standup` (no AI calls from the panel). UI ships prebuilt (`scaffold.js`); wiring per `.github/skills/scaffolding/SKILL.md`, gated on `SCAFFOLD_PANEL=1` |
| `/demolish-scaffold` | Strike the Scaffold — preview-and-confirm removal of the wiring. Queued requests and spec edits kept by default; `scaffold.js` stays (framework-managed) |
